...
function LauncherGamesList(props) {
  ...
  function handleExpandCollapse() {
    if (glIsOpen) {
      ...
      // TODO: Fix this
      setContainerHeight(
        expendContainer.current.querySelector('ul').clientHeight,
      );
      setGlIsOpen(true);
    }
  }
  ...
  return (
    <div className={`expended-launcher-game-list ${glIsOpen ? 'is-open' : ''}`}>
      <button className="expend-btn" onClick={handleExpandCollapse}>
        <svg className="games-icon">
          <use href="/img/sprite.svg#gamesIcon" />
        </svg>
      ...
      <div
        className="launcher-game-list-inner"
        ref={expendContainer}
        style={{ height: `${containerHeight}px` }}
      >
        <ul className="launcher-game-list">{gameList}</ul>

        <button className="hide-button" onClick={handleExpandCollapse}>
          Hide List
        </button>
      </div>
    </div>
